        ...
        <Container className="relative grid items-center gap-10 lg:max-w-[1320px] lg:grid-cols-[0.82fr_1.18fr] lg:gap-14">
          ...
          <Reveal delay={0.08} className="min-w-0">
            <div className="overflow-hidden rounded-[22px] border border-[color:var(--border-subtle)] bg-[color:var(--background-elevated)] shadow-[0_30px_90px_color-mix(in_oklch,var(--primary)_18%,transparent)]">
              <div className="flex h-10 items-center gap-1.5 border-b border-[color:var(--border-subtle)] px-4" aria-hidden="true"><span className="size-2 rounded-full bg-[color:var(--accent)]" /><span className="size-2 rounded-full bg-[color:var(--primary)]/55" /><span className="size-2 rounded-full bg-[color:var(--text-faint)]/35" /><span className="ml-3 h-5 flex-1 rounded-md bg-[color:var(--surface-soft)]" /></div>
              <div className="relative aspect-[16/10] bg-[color:var(--surface)]"><Image src={project.coverImage} alt={project.coverImageAlt || `${project.title} project preview`} fill priority className="object-cover object-top" sizes="(max-width: 1024px) 100vw, 58vw" /></div>
            </div>
          </Reveal>
        </Container>
      </section>

      {facts.length > 0 && <section aria-label="Project facts"><Container className="lg:max-w-[1320px]"><div className="grid grid-cols-2 gap-px overflow-hidden rounded-[18px] border border-[color:var(--border-subtle)] bg-[color:var(--border-subtle)] md:grid-cols-3 lg:grid-cols-5">{facts.map(([label, value, iconKey]) => { const Icon = resolveProjectIcon(iconKey, `${label} ${value}`); return <div key={label} className="min-w-0 bg-[color:var(--surface)] px-4 py-4 sm:px-5"><div className="flex items-start gap-3"><Icon className="mt-0.5 shrink-0 text-[color:var(--primary)]" size={18} aria-hidden="true" /><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[0.08em] text-[color:var(--text-faint)]">{label}</p><p className="mt-1 break-words text-sm font-bold leading-5 text-[color:var(--text-strong)]">{value}</p></div></div></div>})}</div></Container></section>}
...
function OptionalItems({ items, label, heading, className = "" }: { items?: ProjectListItem[]; label: string; heading?: string; className?: string }) {
  ...
  return <section aria-label={label} className={className}><Container className="lg:max-w-[1320px]">{heading && <h2 className="mb-7 font-display text-3xl font-black text-[color:var(--text-strong)]">{heading}</h2>}<div className="grid gap-px overflow-hidden rounded-[18px] border border-[color:var(--border-subtle)] bg-[color:var(--border-subtle)] sm:grid-cols-2 lg:grid-cols-4">{sorted.map((item, index) => { const Icon = resolveProjectIcon(item.iconKey, `${item.title} ${item.description || ""}`); return <Reveal key={item.id || `${item.title}-${index}`} delay={index * 0.035}><div className="h-full bg-[color:var(--surface)] p-5"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-[color:var(--primary)]/12 text-[color:var(--primary)]"><Icon size={17} aria-hidden="true" /></span><div><h3 className="font-display text-sm font-black leading-5 text-[color:var(--text-strong)]">{item.title}</h3>{item.description && <p className="mt-2 text-xs leading-5 text-[color:var(--text-muted)]">{item.description}</p>}</div></div></div></Reveal>})}</div></Container></section>;
}
